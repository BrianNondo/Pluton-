...
// In-memory store for recent messages (use a database for persistence)
const userConversations = {};

async function handleUserMessage(userId, userMessage, chatId, sock) {
  try {
    ...
    if (userConversations[userId].length > 3) {
      userConversations[userId] = userConversations[userId].slice(-3);
    }

    // Combine messages into one query (optional but improves response)
    const query = userConversations[userId].join(" ");

    // 🔥 CALL YOUR API
    const res = await axios.get(
      `https://ab-deepseekai.abrahamdw882.workers.dev/?q=${encodeURIComponent(query)}`
    );

    let data = res.data;

    // ✅ FIX: Handle string response safely
    if (typeof data === "string") {
      try {
        data = JSON.parse(data);
      } catch (e) {
        console.error("JSON Parse Error:", e.message);
        return await sock.sendMessage(chatId, { text: "Error parsing AI response." });
      }
    }

    // ✅ Extract ONLY the response field
    let reply = data.response || "No response found.";

    // 🔥 OPTIONAL: Clean unwanted "thinking"
    reply = reply.split("The assistant acknowledges")[0].trim();

    // Store assistant reply
    userConversations[userId].push(reply);
    ...
    await sock.sendMessage(chatId, {
      text: "Sorry, there was an error talking to the AI."
    });
  }
}
...
const channelInfo = {
    contextInfo: {
        ...
        forwardedNewsletterMessageInfo: {
            ...
            newsletterName: 'pluton news letter',
            serverMessageId: -1
        }
    }
};
...
async function handleMessages(sock, messageUpdate, printLog) {
    try {
        ...
        if (/^[1-9]$/.test(userMessage) || userMessage.toLowerCase() === 'surrender') {
          ...
          if (!isGroup) {
            await handleUserMessage(senderId, userMessage, chatId, sock);
            }
